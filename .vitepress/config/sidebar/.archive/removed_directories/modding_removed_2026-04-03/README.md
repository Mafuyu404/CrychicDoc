# Archived Directory: modding

**Archive Date**: 2026-04-03T13:40:16.363Z
**Original Location**: develop/modding/modding
**Reason**: Physical directory no longer exists in docs structure

## Contents
This archive contains both the configuration files and metadata for a directory that was removed from the physical docs structure.

- `config/` - Contains the JSON configuration files (locales.json, order.json, etc.)
- `metadata/` - Contains the metadata files tracking configuration history

## Restoration
To restore this directory:

1. **Recreate the physical directory**: 
   `mkdir -p docs/en/develop/modding/modding/`

2. **Restore configuration files**:
   `cp -r config/modding/ .vitepress/config/sidebar/en/develop/modding/modding/`

3. **Restore metadata files**:
   `cp -r metadata/modding/ .vitepress/config/sidebar/.metadata/en/develop/modding/modding/`

4. **Restart the development server**

## Archive Structure
```
modding_removed_2026-04-03/
├── README.md (this file)
├── config/modding/     # Original config files
└── metadata/modding/   # Original metadata files
```
