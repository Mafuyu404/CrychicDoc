 ...
 *
 * @module utils/config/nav-config
 * @description
 ...
 * @example
 * ```ts
 * import { navConfig, type NavItem } from "@utils/config/nav-config";
 * const enNav = navConfig.locales["en-US"]; // NavItem[]
 * ```
 */

import { projectConfig } from "./project-config";
import type { NavConfig, NavItem } from "./nav-types";
 ...
 * - `useBreadcrumb.ts`    — breadcrumb trail generation
 *
 * Internal `link` fields are root-relative **without** a locale prefix.
 * Use `prefixNavLinks` from `nav-link-access.ts` to prepend the active locale
 * path at runtime.
 */
