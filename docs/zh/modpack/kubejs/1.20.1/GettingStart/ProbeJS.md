...
在 KubeJS 开发的任何阶段，`ProbeJS` 都是必要的辅助工具。

它做的事情并不复杂：把当前实例里一部分运行时信息生成编辑器能够识别的类型文件，让 `VSCode` 可以补全事件、对象与方法。这样你写脚本时，就不必一直靠记忆或盲猜。
...
- 在诸如 `event.player` 这样的对象后输入 `.`，可以继续查看该对象下的函数与字段。
- 在需要传入物品、方块、实体、维度等标识符时，编辑器也会给出更合适的补全候选。

这样做能把精力留给真正的逻辑问题。至于补全本身有哪些边界、为什么有些对象看起来不够“完整”，则会在后面的章节里继续展开。
...
这段注释不会改变运行时对象，它只是告诉编辑器：这里按 `Internal.ServerPlayer` 来理解。这样原本补不出来的方法才会重新出现。如果你发现代码中的某个实体没有药水效果或属性相关的函数与字段，也可以尝试先把类型收紧到更合适的层级，再重新观察补全结果。
