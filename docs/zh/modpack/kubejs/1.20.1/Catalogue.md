...
下面用 `message` 组件演示一次无效提问和一次有效提问的差别：
...
::: message nickname="提问者" avatar-type="icon"
Minecraft `1.20.1`，Forge，KubeJS `6`，ProbeJS Legacy。脚本在 `server_scripts`。我想在 `PlayerEvents.loggedIn` 时给玩家显示动作栏消息。当前代码是 `PlayerEvents.loggedIn(event => { event.player.showActionbar(Text.green('hi')) })`。进入世界后没有报错，也没有显示消息。我已经重新 `/probejs dump`，并确认脚本会重载。接下来我应该先查事件、js文件的位置还是 `player` 的类型？
:::

::: message nickname="社区成员" avatar-type="icon" location="right"
这样的问题就能继续排查了。下一步先确认事件有没有实际执行，再看动作栏调用、日志输出和类型补全；如果补全提供的是父类，需要补一下 `JSDoc` 来使用正确的类型。
:::
::::

**当前的 AI 在 KubeJS 开发上幻觉和误差都很大，不推荐依赖。**但可以使用。
